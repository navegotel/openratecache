...
//StrToTime takes a date string in rate header format and returns
//a time.Time object.
...
// PackRate packs rate and availability into a single uint32
// and returns the value as a 4 byte string (big endian)
// that can be written to the rate cache.
...
// UnpackRate takes a 4 byte string, unpacks values for rate
// and availability and returns them separately.
...
// OccupancyItem represents a guest type identified by an age range that
...
// ToByteStr returns a representation of OccupancyItem as byte string
...
// FileHeader represents header information from a rate cache file which can be
...
func FileHeaderFromByteStr(byteStr []byte) (*FileHeader, error) {
	...
	if int(byteStr[16]) != Version {
		return nil, fmt.Errorf("Wrong version. expected version is %d, got %d", Version, int(byteStr[16]))
	}
...
// RateBlockHeader represents a rate block header information which can be read
