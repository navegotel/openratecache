...
//On possible occupancy for a room or room rate. idx points to the
//rate block in the cache file
...
//Return a nested map as used as cache index.
func InitIdx() map[string]map[string][]RoomOccIdx {
    m := make(map[string]map[string][]RoomOccIdx)
    return m
}

//Add RoomOccIdx to index map. Room is added if not exists,
